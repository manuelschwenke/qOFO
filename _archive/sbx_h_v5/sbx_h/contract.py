...
class CorridorContract:
    ...
    """Optional PLANNING-DERIVED tier-1 band schedule (v3 closing
    piece, 2026-07-08): ordered ``(t_from_s, q_band_mvar)`` intervals —
    per hour, the band the day-ahead calculation itself derives
    (forecast-ensemble spread + tracking + model-gap terms, see
    ``experiments/017_SBX_PLANNING.py --band-ensemble``).  Same
    interval rules as ``v_std_schedule``; the constant ``q_band_mvar``
    field must equal the first interval.  ``None`` keeps the constant
    band."""
    ...
    def q_band_at(self, time_s: float) -> float:
        ...
        (v3 closing piece; the constant band without a schedule)."""
        if not math.isfinite(time_s) or time_s < 0.0:
...
def build_default_contract(
    ...
    *,
    ...
    if q_band_schedule is not None:
        ...
        # The constant field is the t = 0 view of the band schedule.
