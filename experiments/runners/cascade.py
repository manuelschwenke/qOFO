...
# -*- coding: utf-8 -*-
"""
experiments/runners/cascade.py
==============================
Single-TSO / single-DSO cascaded OFO simulation entry point.

This module hosts :func:`run_cascade`, the cascaded TSO-DSO main loop that
operates on a single combined network (``build_tuda_net``).  Extracted from
``experiments/001_S_TSO_S_DSO.py`` so the experiment script can stay focused
on configuration and post-run analysis.

Architecture
------------
* TSO every 3 min, DSO every 1 min on a combined network.
...
* TSO sends Q setpoints to DSO at the PCCs; the DSO returns capability
  messages.

All shared helpers (contingency injection, plant-IO, sensitivity
construction, record dataclasses) live in :mod:`experiments.helpers` and
are imported here unchanged.
