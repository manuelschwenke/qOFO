...
def main() -> int:
    ...
    ap.add_argument("--scenario", type=str, default="asym_z1",
    ...
    ap.add_argument("--schedule", type=str, default="auto",
                    help="path to a planning-anchored v_std/band "
                         "schedule JSON (SBX v3, from experiments/017_"
                         "SBX_PLANNING.py). Default 'auto' picks the "
                         "017 default output when it exists; "
                         "--no-schedule forces the v2 snapshot contract "
                         "(30-min warmup).")
    ap.add_argument("--no-schedule", action="store_true",
                    help="v2 snapshot contract (settled-state freeze "
                         "after the 30-min warmup) instead of the "
                         "planning schedule")
    ap.add_argument("--shared-sens", action="store_true",
                    help="use the shared full-network Jacobian instead "
                         "of the default LOCAL cached sensitivities "
                         "(Ward-style reduced zone nets — the "
                         "SBX-consistent configuration)")
    ...
    if not args.shared_sens:
        # DEFAULT: zones control from their own reduced-net cached
        # models — the SBX-consistent configuration (locality is the
        # design premise). --shared-sens restores the full-network
        # Jacobian path (013 campaign comparability).
        cfg.local_sensitivities_tso = True
        cfg.local_sensitivities_dso = True
    print(f"    sensitivities: "
          f"{'shared full-network' if args.shared_sens else 'LOCAL'}")

    # v3 planning schedule (DEFAULT when the 017 output exists): the
    # warmup existed only for the settled-state snapshot (revised A7);
    # with a schedule the mechanism (and Figure 6) arms at t = 0.
    sched_path = None
    if not args.no_schedule:
        if args.schedule == "auto":
            cand = (REPO / "results" / "017_SBX_PLANNING"
                    / "schedule_perfect_360min.json")
            if cand.exists():
                sched_path = cand
            else:
                print("    NOTE: no planning schedule found (run "
                      "experiments/017_SBX_PLANNING.py) — falling back "
                      "to the v2 snapshot contract (30-min warmup).")
        else:
    ...
    if sched_path is not None:
        ...
        print("    contract: v2 snapshot (freeze at "
              f"{cfg.sbx_warmup_s / 60.0:.0f} min)")
    ...
    # Full run payload (013-style pickle) so post-hoc analysis (015
    # plots, exposure numbers) never needs a rerun.  Re-create the
    # output directory: long runs have seen it swept away by result
    # clean-ups in the meantime.
    out_dir.mkdir(parents=True, exist_ok=True)
    import pickle
    pkl = out_dir / f"run_{args.arm}.pkl"
    with open(pkl, "wb") as fh:
        pickle.dump({"scenario": scenario, "arm": args.arm,
                     "minutes": args.minutes, "records": recs,
                     "sbx": _013.extract_sbx_runtime(adapter),
                     "q_band_mvar": band,
                     "schedule": (str(sched_path) if sched_path
                                  else None),
                     "local_sens": not args.shared_sens}, fh)
    print(f"    payload: {pkl}")
