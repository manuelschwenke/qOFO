...
from experiments.results_io import latest_run_dir, new_run_dir  # noqa: E402
...
def main() -> int:
    ...
    if args.run:
        snapshots = {
            f"{cell}:{arm}": make_config(cell, arm, args.minutes)
            for cell in cells
            for arm in ARMS
        }
        RESULT_DIR = new_run_dir(
            "comparison_none_vs_sbxh", snapshots
        ).root
    elif args.evaluate:
        RESULT_DIR = latest_run_dir(
            "comparison_none_vs_sbxh"
        ).root
