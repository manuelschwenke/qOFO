...
def _encode_warm_start(
    ...
    ``tune_multi_zone``); the DSO block reproduces Phase 1 of
    ``tune_dso``.  Then, optimal alpha values are derived from the
    resulting M matrices.  This guarantees that PSO starts from a point
    at least as good as what the legacy tuner would have produced.
    """
    ...
    for idx, z in enumerate(zone_ids):
        ...
        # Refine alpha to its local optimum given this g_w
    ...
    for d_idx, d in enumerate(dso_inputs):
        ...
        # Refine DSO alpha to its local optimum
        _, _, alpha_opt = _dso_cascade_decay(d.H, d.q_obj_diag, gw, n_inner)
        alpha_dso[d_idx] = alpha_opt
...
def _evaluate_fitness(
    ...
    *,
    ...
    """Compute the min-max fitness for one PSO particle.
    ...
    try:
        ...
        alpha_eff_tso = max(alpha_tso) if alpha_tso else 0.0
        spectral = alpha_eff_tso * float(stab.M_sys_lambda_max)
        s_half = 0.5 * spectral
        gamma = float(stab.small_gain_gamma)

        # DSO blocks: per-DSO cascade decay
        max_decay = 0.0
        per_dso = []
        for d_idx, d in enumerate(dso_inputs):
            ...
            per_dso.append((rho_d, decay_d))
            if decay_d > max_decay:
                max_decay = decay_d

        fitness = float(max(s_half, gamma, max_decay))
        ...
        metrics = {
            'fitness': fitness,
            'spectral': spectral,
            'spectral_half': s_half,
            'gamma': gamma,
            'max_dso_decay': max_decay,
            'alpha_eff_tso': alpha_eff_tso,
            'alpha_eff_dso': max(alpha_dso) if alpha_dso else 0.0,
        }
        return fitness, metrics, stab

    except (np.linalg.LinAlgError, FloatingPointError, ValueError) as exc:
        return 1e6, {'fitness': 1e6, 'error': str(exc)}, None
    ...
    for it in range(1, max_iterations + 1):
        ...
        if verbose:
            print(
                ...
                f"s_half={m_best.get('spectral_half', float('nan')):.4f}  "
                f"gamma={m_best.get('gamma', float('nan')):.4f}  "
                f"max_dso={m_best.get('max_dso_decay', float('nan')):.4f}  "
                f"w={w:.3f}"
            )
...
def tune_pso_all(
    *,
    ...
    if stab_final.small_gain_gamma >= 1.0:
        feasibility_warnings.append(
            f"TSO small-gain gamma = {stab_final.small_gain_gamma:.4f} "
            f">= 1 (Lyapunov non-contraction)"
        )
