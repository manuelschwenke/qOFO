...
(``sbx_h.messages.assert_checksums_match``), ``rep1`` on mismatch.
...
from sbx_h.messages import PeerCairMessage
...
def match(
    ...
    for msg in (msg_a, msg_b):
        if msg.request_mvar is None:
            continue
        # v5 sized requests (2026-07-10): the magnitude must be a
        # POSITIVE INTEGER MULTIPLE of the per-cycle quantum (plan
        # §2.2 Step 2 amended; k = 1 reproduces the v4 rule).
        k = abs(msg.request_mvar) / quantum
        if not math.isclose(k, round(k), rel_tol=0.0, abs_tol=1e-9) \
                or round(k) < 1:
            rep1("request magnitude must be a positive integer multiple "
                 "of the per-cycle quantum (plan §2.2 Step 2, v5 sized "
                 "requests)",
