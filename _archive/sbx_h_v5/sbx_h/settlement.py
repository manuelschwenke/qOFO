...
* **Tier 2 (surplus transfer).**  The scheduler decomposes the surplus
  ...
  ``sbx_h.scheduler``).  The PAID portion is billed
...
class CycleObservation:
    ...
    q_band_mvar: float
    """Tier-1 band ACTIVE during the observed cycle (v3: hourly band
    schedules make this a per-cycle quantity; the scheduler captures it
    with the rest of the elapsed-cycle state)."""

    delivered: Optional[bool] = None
    """v5 (2026-07-10): voltage-referenced delivery verification of
    this cycle — did the ACTING side's terminals track their shifted
    references?  ``None`` when no surplus was scheduled (nothing to
    verify) or for pre-v5 producers.  Consumed by the tier-2 billing
    when ``SBXConfig.tier2_requires_delivery`` is set."""
...
class CycleSettlement:
    ...
    #: v5 (2026-07-10): fraction of the window's paid surplus that was
    #: BILLED after the delivery gate (1.0 = fully billed / v4;
    #: 0.0 = undelivered, tier 2 suspended for the window).
...
class SettlementEngine:
    ...
    def observe(self, obs: CycleObservation) -> CycleSettlement:
        ...
        # v3: the band is a per-cycle quantity (hourly schedules);
        # windowed like every other settled quantity.
        ...
        # v5: no payment for undelivered support — each window cycle's
        # paid surplus is billed only when its voltage-referenced
        # delivery verification succeeded (None = nothing scheduled /
        # pre-v5 producer → treated as delivered, the v4 behaviour).
        delivered_frac = 1.0
        if self.config.tier2_requires_delivery:
            delivered_frac = float(np.mean(
                [0.0 if o.delivered is False else 1.0 for o in w]
            ))
