...
class TestDSOController:
    ...
    def test_step_infeasible_raises(self, dso_setup) -> None:
        """Placeholder: requires a no-slack solver to actually trigger infeasibility.

        TODO: Construct an MIQP variant with G_z = 0 (hard constraints) and
        contradictory bounds, then assert that controller.step() raises
        RuntimeError. As long as soft slack is enabled the solver always
        returns a feasible point, so this test cannot exercise the raise path.
        """
        pytest.skip("Infeasibility path needs no-slack solver fixture; see TODO.")
...
class TestTSOController:
    ...
    def test_output_limits_without_setpoints(self, tso_setup) -> None:
        ...
        # Output ordering: [V_bus | Q_PCC | I_line | Q_gen]  (Strategy D)
        assert len(y_lo) == n_v + n_pcc + n_i + n_gen
        # Voltage: general band
        assert_allclose(y_lo[:n_v], config.v_min_pu)
        assert_allclose(y_hi[:n_v], config.v_max_pu)
        # Q_PCC band: with output-side bound enabled (default), the band
        # is centred at q_iface_now ± DSO capability (which is wide at
        # init: ±1e6), so the absolute bounds are wide.
        assert y_lo[n_v] < y_hi[n_v]
        # Current: [0, 1E6] (no per-line ratings provided)
        assert_allclose(y_lo[n_v + n_pcc:n_v + n_pcc + n_i], 0.0)
        assert_allclose(y_hi[n_v + n_pcc:n_v + n_pcc + n_i], 1E6)
    ...
    def test_sensitivity_matrix_shape(self, tso_setup) -> None:
        ...
        # Output ordering (Strategy D, Q_PCC re-enabled):
        # [V_bus(3) | Q_PCC(1) | I_line(1) | Q_gen(0)]
        n_outputs = 3 + 1 + 1 + 0
    ...
    def test_sensitivity_pcc_column_has_voltage_effect(self, tso_setup) -> None:
        """Test that PCC setpoint column has both V and Q_PCC sensitivities.

        Strategy D (Q_PCC rows re-enabled): the PCC column now holds
        ∂V/∂Q_PCC,set (negated for load convention) in the V-row block,
        AND a unit identity entry in the Q_PCC row block (closed-loop
        view: this DSO tracks its own setpoint).
        """
        controller, _, measurement = tso_setup
        controller.initialise(measurement)
        ...
        # Output rows: [V(3) | Q_PCC(1) | I(1) | Q_gen(0)]
        assert H.shape[0] == n_v + n_pcc + 1 + 0
        # V rows: PCC column has non-zero ∂V/∂Q_inj at PCC HV bus
        assert not np.allclose(H[:n_v, pcc_col], 0.0)
        # Q_PCC row: identity 1.0 on the diagonal (closed-loop view)
        assert H[n_v, pcc_col] == 1.0
