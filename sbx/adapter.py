...
the plant-agnostic :class:`~sbx.scheduler.SBXScheduler` interface, and
...
1. On a cycle boundary (``iteration % k_sched == 0``, iteration > 0):
   build :class:`~sbx.scheduler.AreaCycleData` per area from the
...
from sbx.contract import build_default_contract
from sbx.corridor import build_corridor_registry
from sbx.fail import rep1
from sbx.scheduler import AreaCycleData, AreaStepInput, SBXScheduler
        ...
        for z in self.scheduler.area_ids:
            ...
            if ctrl.config.g_v <= 0.0:
                rep1("SBX requires a positive voltage-tracking weight "
                     "g_v (w_track ≡ g_v, A4/G5)", area=z,
                     g_v=ctrl.config.g_v)
            if config.w_track is not None and not math.isclose(
                    config.w_track, float(ctrl.config.g_v)):
                rep1("SBXConfig.w_track must equal the zone's g_v — "
                     "per-bus tracking weights would require touching "
                     "the MIQP assembly (A4/G5)", area=z,
                     w_track=config.w_track, g_v=ctrl.config.g_v)
