...
def apply_zone_tso_controls(
    ...
    The DER block is written into ``net.sgen.q_set_mvar`` — the central Q
    setpoint (sgen sign convention, positive = inject) that the plant-side
    :class:`controller.der_qv_local_loop.QVLocalLoop` feeds in directly
    while V stays inside the deadband, with linear droop overriding when V
    exits the deadband.
    ...
    prev_shunt_steps : List[int]
        ...
        ``ShuntDisturbanceMessage`` to the affected DSO controllers accordingly.
    """
    u = tso_out.u_new
...
def apply_dso_controls(
    ...
    The DER block is written into ``net.sgen.q_set_mvar`` — the central Q
    setpoint (sgen sign convention, positive = inject) consumed by the
    plant-side :class:`controller.der_qv_local_loop.QVLocalLoop`.  Inside
    the deadband the realised Q equals ``q_set_mvar``; outside the
    deadband the local droop overrides.

    The ``sensitivities`` argument is unused under refactor_v3 (the
    Q-shim S_VQ inversion path was removed) but is retained in the
    signature so existing call sites keep working.
    """
    del sensitivities  # unused after the Q-shim path removal
    u = dso_out.u_new
