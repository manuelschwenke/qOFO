...
from sbx_h.corridor import build_corridor_registry, corridor_q_flow, \
    corridor_sensitivities, corridor_solve_dv
...
def test_golden_2_corridor_round_trip(case):
    net, registry = case
    for corr in registry.values():
        v_a, v_b, p_a, q_a = _corridor_measurements(net, corr)
        q_base = sum(q_a)
        for acting_end in ("a", "b"):
            for q_target in (q_base + 10.0, q_base - 10.0, q_base):
                dv = corridor_solve_dv(
                    corr, v_a, v_b, p_a, q_target, acting_end
                )
                if acting_end == "a":
                    va = [v + dv for v in v_a]
                    vb = list(v_b)
                else:
                    va = list(v_a)
                    vb = [v + dv for v in v_b]
                q_back = corridor_q_flow(corr, va, vb, p_a)
                assert abs(q_back - q_target) <= 1e-6, (
                    f"corridor {(corr.area_a, corr.area_b)} acting "
                    f"{acting_end}: round trip error "
                    f"{abs(q_back - q_target):.2e} Mvar"
                )
