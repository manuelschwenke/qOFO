...
| 6 — Evaluation ladder + Monte Carlo | 🚧 6a ✅ (w_Φ = 1e5) + 6b ✅ (D6: ε = 5.2e3, c = 1.0e3/5.2e3) + 6c ✅ (D2: edges (1.01, 1.05), w_band = 1e4) + 6d ✅ (oracle rung = SINGLE-ZONE BME oracle via `single_zone_partition=True`, D8 "both") + 6e ✅ (metrics completion: METRIC-objective split — `bme_metric_w_band`, 3-area Φ_i on every rung, `bme_v_boundary` signal; gap-to-oracle, Phulpin overcost, AR(2) oscillation indicator; consistent 120-min ladder re-run — KEY FINDING: bme ≈ oracle on losses (−2.95 %, 2 kW apart) but the last-hour Φ ranking inverts (none < bme < oracle) via band-edge-riding; discuss calibration philosophy with Manuel BEFORE the MC campaign) — next: (6) MC campaign. Calibration sweeps: 120-min horizon (Manuel) | see BME_STATUS.md §6e |
