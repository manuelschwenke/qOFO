...
from experiments.results_io import latest_run_dir, new_run_dir
...
OUT_ROOT = os.environ.get(
    "QOFO_CIGRE006_RUN_DIR", results_path("006_cigre_mc")
)
...
def _set_output_root(root) -> None:
    """Update all output globals after selecting a numbered run."""
    global OUT_ROOT, TS_DIR, SCRATCH_DIR
    global SCHED_CSV, METRICS_CSV, FAILED_CSV, RUNS_DIR
    OUT_ROOT = str(root)
    TS_DIR = os.path.join(OUT_ROOT, "timeseries")
    SCRATCH_DIR = os.path.join(OUT_ROOT, "_scratch")
    SCHED_CSV = os.path.join(OUT_ROOT, "contingency_schedules.csv")
    ...
    FAILED_CSV = os.path.join(OUT_ROOT, "failed_attempts.csv")
    RUNS_DIR = os.path.join(OUT_ROOT, "_runs")
    os.environ["QOFO_CIGRE006_RUN_DIR"] = OUT_ROOT
...
def main() -> None:
    ...
    if args.replot or args.rerun_variant or args.resume:
        _set_output_root(latest_run_dir("cigre_2026_006").root)
    else:
        snapshot = {
            "config": make_cigre_config(),
            "runs": args.runs,
            "seed": args.seed,
            "jobs": args.jobs,
        ...
        _set_output_root(
            new_run_dir("cigre_2026_006", snapshot).root
        )
    os.makedirs(OUT_ROOT, exist_ok=True)
    if args.replot:
        print("=" * 72)
